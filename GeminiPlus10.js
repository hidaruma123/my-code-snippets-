...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def create_widgets(self):
        ...
        # ★★★ Mirrorアクションを追加 ★★★
        self.action_mirror = self.toolbar.addAction("Mirror Selected")
    ...
    def create_connections(self):
        ...
        # ★★★ Mirrorアクションの接続 ★★★
        self.action_mirror.triggered.connect(self.mirror_selected_button)
    ...
    def toggle_edit_mode(self, checked):
        ...
        # ★★★ Mirrorアクションは編集モード時のみ有効 ★★★
        self.action_mirror.setEnabled(checked)
    ...
    # ★★★ 新しいメソッド: 選択されたボタンをミラー ★★★
    def mirror_selected_button(self):
        if not self.action_edit_mode.isChecked():
            QtWidgets.QMessageBox.warning(self, "Warning", "Mirror function is only available in Edit Mode.")
            return
        
        selected_buttons = [btn for btn in self.picker_buttons if btn.is_selected]
        if len(selected_buttons) != 1:
            QtWidgets.QMessageBox.warning(self, "Warning", "Please select exactly one button to mirror.")
            return
        
        original_button = selected_buttons[0]
        original_controller = original_button.controller
        ...
        if original_controller.startswith("Con_L_"):
            ...
            QtWidgets.QMessageBox.warning(self, "Warning", "Selected controller does not start with 'Con_L_' or 'Con_R_'.")
            return
        
        # ミラーコントローラーが存在するか確認
        if not cmds.objExists(mirrored_controller):
            QtWidgets.QMessageBox.warning(self, "Warning", "Mirrored controller '{}' does not exist in the scene.".format(mirrored_controller))
            return
        ...
        # 元のボタンを選択解除し、新しいボタンを選択
        original_button.is_selected = False
        original_button.update_style()
        mirrored_button.is_selected = True
        mirrored_button.update_style()
        
        QtWidgets.QMessageBox.information(self, "Success", "Mirrored button created for '{}'.".format(mirrored_controller))
