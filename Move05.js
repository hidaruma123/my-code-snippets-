...
class PickerButton(QtWidgets.QPushButton):
    ...
    def __init__(self, controller, color, shape, width, height, custom_text="", parent=None):
        ...
        self.parent_tool = None  # ★★★ 親ツールへの参照を保存 ★★★
    ...
    def mousePressEvent(self, event):
        if self.is_draggable:
            if event.button() == QtCore.Qt.LeftButton:
                ...
                # ★★★ 複数選択時の初期位置を記録 ★★★
                if self.is_selected and self.parent_tool:
                    self.parent_tool.start_multi_move(self)
        ...
        if (event.buttons() == QtCore.Qt.LeftButton and self.drag_start_position is not None and self.is_draggable):
            if (event.pos() - self.drag_start_position).manhattanLength() >= QtWidgets.QApplication.startDragDistance():
                ...
                if self.is_selected and self.parent_tool:
                    ...
                    self.parent_tool.move_selected_buttons(delta)
                else:
    ...
    def mouseReleaseEvent(self, event):
        ...
        # ★★★ 移動終了を通知 ★★★
        if self.parent_tool:
            self.parent_tool.end_multi_move()
...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def add_picker_button(self):
        ...
        button.parent_tool = self  # ★★★ 親ツールへの参照を設定 ★★★
    ...
    # ★★★ 複数選択移動の開始 ★★★
    def start_multi_move(self, dragged_button):
        if not self.multi_move_active:
            self.multi_move_active = True
            self.multi_move_initial_positions = {}
            for button in self.picker_buttons:
    ...
    # ★★★ 選択されたボタンを移動 ★★★
    def move_selected_buttons(self, delta):
        if self.multi_move_active and self.multi_move_initial_positions:
            for button, initial_pos in self.multi_move_initial_positions.items():
                new_pos = initial_pos + delta
                button.move(new_pos)

    # ★★★ 複数選択移動の終了 ★★★
    def end_multi_move(self):
        self.multi_move_active = False
        self.multi_move_initial_positions = {}
    ...
    def mirror_selected_button(self):
        ...
        for original_button, mirrored_controller in mirrorable_buttons:
            ...
            mirrored_button.parent_tool = self  # ★★★ 親ツールへの参照を設定 ★★★
    ...
    def load_picker(self):
        ...
        for btn_data in data.get('buttons', []):
            ...
            btn = PickerButton(btn_data['controller'], btn_data['color'], btn_data['shape'],
                               width, height, custom_text, self.picker_area_bg_label)
            btn.parent_tool = self  # ★★★ 親ツールへの参照を設定 ★★★
