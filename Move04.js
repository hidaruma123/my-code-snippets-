...
class PickerButton(QtWidgets.QPushButton):
    ...
    # ★★★ 複数選択移動のためのシグナルを追加 ★★★
    multi_move_request = QtCore.Signal(object, object)
        ...
        if (event.buttons() == QtCore.Qt.LeftButton and self.drag_start_position is not None and self.is_draggable):
            if (event.pos() - self.drag_start_position).manhattanLength() >= QtWidgets.QApplication.startDragDistance():
                ...
                if self.is_selected:
                    ...
                    self.multi_move_request.emit(self, delta)
                else:
...
class RigPickerTool(QtWidgets.QMainWindow):
    def __init__(self, parent=get_maya_main_window()):
        ...
        # ★★★ 複数選択移動用の変数を追加 ★★★
        self.multi_move_initial_positions = {}
    ...
    def add_picker_button(self):
        ...
        # ★★★ 複数選択移動のシグナルを接続 ★★★
        button.multi_move_request.connect(self.handle_multi_move)
        button.move(10, 10)
        button.show()
        self.picker_buttons.append(button)

    # ★★★ 複数選択移動を処理する新しいメソッド ★★★
    def handle_multi_move(self, dragged_button, delta):
        # 初回のドラッグ時に初期位置を記録
        if not self.multi_move_initial_positions:
            for button in self.picker_buttons:
                if button.is_selected:
                    self.multi_move_initial_positions[button] = button.pos()
        
        # 選択された全てのボタンを移動
        if self.multi_move_initial_positions:
            for button, initial_pos in self.multi_move_initial_positions.items():
                if button == dragged_button:
                    # ドラッグ中のボタンは直接移動
                    new_pos = button.mapToParent(button.mapFromParent(initial_pos) + delta)
                    button.move(new_pos)
                else:
                    # 他のボタンは相対位置を保って移動
    ...
    def mirror_selected_button(self):
        ...
        for original_button, mirrored_controller in mirrorable_buttons:
            ...
            # ★★★ 複数選択移動のシグナルを接続 ★★★
            mirrored_button.multi_move_request.connect(self.handle_multi_move)
    ...
    def load_picker(self):
        ...
        for btn_data in data.get('buttons', []):
            ...
            # ★★★ 複数選択移動のシグナルを接続 ★★★
            btn.multi_move_request.connect(self.handle_multi_move)
    ...
    # ★★★ マウスリリース時に初期位置をリセットするためのイベントフィルター ★★★
    def eventFilter(self, obj, event):
        if event.type() == QtCore.QEvent.MouseButtonRelease:
            self.multi_move_initial_positions = {}
        return super(RigPickerTool, self).eventFilter(obj, event)
