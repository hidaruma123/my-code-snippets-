...
# ★★★ 背景画像とラバーバンドの機能をすべて内包した、自己完結型のラベルクラス ★★★
class PickerAreaWidget(QtWidgets.QLabel):
    def __init__(self, parent_tool):
        super(PickerAreaWidget, self).__init__()
        ...
        self.clear_background_image() # 初期テキストとスタイルを設定
    ...
    def mousePressEvent(self, event):
        ...
        super(PickerAreaWidget, self).mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event):
        ...
        super(PickerAreaWidget, self).mouseMoveEvent(event)
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton and not self.selection_origin.isNull():
            ...
            self.selection_origin = QtCore.QPoint()
        super(PickerAreaWidget, self).mouseReleaseEvent(event)
    ...
    def clear_background_image(self):
        self.background_pixmap = None
        self.background_image_path = None
        self.setText("Right-click to set background image\nDrag on empty area to select multiple")
        self.setStyleSheet("border: 2px dashed #555; color: #777;")

    def update_background(self):
        if self.background_pixmap and not self.background_pixmap.isNull():
            scaled_pixmap = self.background_pixmap.scaled(self.size(), QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
            self.setPixmap(scaled_pixmap)
            self.setText("")
            self.setStyleSheet("border: none;")

    # ★★★ このウィジェット自身のサイズが変更されたときに自動で呼ばれるイベント ★★★
    def resizeEvent(self, event):
        super(PickerAreaWidget, self).resizeEvent(event)
        self.update_background() # ここで背景を再描画するのがキー
...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.picker_area_widget = PickerAreaWidget(self)
        self.picker_area_widget.setMinimumSize(600, 600)
        self.picker_area_layout.addWidget(self.picker_area_widget)
    ...
    def create_connections(self):
        ...
        self.picker_area_widget.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.picker_area_widget.customContextMenuRequested.connect(self.show_bg_context_menu)
    ...
    def add_picker_button(self):
        ...
        height = self.height_spin.value() if shape == "Rectangle" else width
        button = PickerButton(
            controller, self.current_color, shape,
            width, height, self.picker_area_widget
        )
    ...
    def save_picker(self):
        ...
        if not path: return
        data = {'buttons': [], 'background_image': self.picker_area_widget.background_image_path}
        for btn in self.picker_buttons:
    ...
    def load_picker(self):
        ...
        if bg_path and os.path.exists(bg_path): self.picker_area_widget.set_background_image(bg_path)
    ...
    def clear_picker(self):
        ...
        self.picker_buttons = []
        self.picker_area_widget.clear_background_image()
    ...
    def show_bg_context_menu(self, pos):
        ...
        action = menu.exec_(self.picker_area_widget.mapToGlobal(pos))
        if action == set_action: self.prompt_set_background_image()
        elif action == clear_action: self.picker_area_widget.clear_background_image()
    ...
    def prompt_set_background_image(self):
        ...
        if path: self.picker_area_widget.set_background_image(path)
