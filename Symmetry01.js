...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.key_selected_btn = QtWidgets.QPushButton("Key Selected")
        self.key_selected_btn.setToolTip("Set keyframes on selected controllers")
    ...
    def create_layouts(self):
        ...
        runtime_actions_layout.addWidget(self.key_selected_btn)
    ...
    def create_connections(self):
        ...
        self.key_selected_btn.clicked.connect(self.key_selected_controllers)
    ...
    def mirror_selected_values(self):
        ...
        # 結果メッセージ
        message = "Successfully mirrored {} controller(s).".format(mirrored_count)
        if failed_controllers:
            message += "\nFailed to mirror: {}".format(", ".join(failed_controllers))
        
        QtWidgets.QMessageBox.information(self, "Mirror Values Complete", message)
    ...
    def reset_selected_controllers(self):
        ...
        QtWidgets.QMessageBox.information(self, "Reset Complete", "Reset {} controller(s).".format(len(selected_controllers)))

    # ★★★ 選択されたコントローラーにキーを設定 ★★★
    def key_selected_controllers(self):
        selected_controllers = cmds.ls(selection=True, type='transform')
        if not selected_controllers:
            QtWidgets.QMessageBox.warning(self, "Warning", "Please select at least one controller.")
            return
        
        cmds.setKeyframe(selected_controllers)
        QtWidgets.QMessageBox.information(self, "Key Complete", "Set keys on {} controller(s).".format(len(selected_controllers)))
