...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def refresh_controller_list(self):
        ...
        for curve in all_curves:
            transform = cmds.listRelatives(curve, parent=True, type='transform', fullPath=True)
            if transform:
                curve_transforms.extend(transform)
        
        # 重複を削除してショート名に変換
        curve_transforms = list(set(curve_transforms))
        curve_transforms = [t.split('|')[-1] for t in curve_transforms]
        
        # パターンに一致するカーブトランスフォームのみをフィルタリング
        controllers = []
        for transform in curve_transforms:
            for pattern in patterns:
                if cmds.filterExpand(transform, sm=12):  # sm=12 は transform を示す
                    if cmds.ls(transform, patterns=pattern):
                        controllers.append(transform)
                        break
