...
class RigPickerCreator(QtWidgets.QMainWindow):
    ...
    def choose_button_color(self):
        """ボタンの色を選択"""
        color = QtWidgets.QColorDialog.getColor(QtGui.QColor(self.current_color))
        if color.isValid():
            self.current_color = color.name()
            self.button_color.setStyleSheet(f"background-color: {self.current_color}")
    ...
    def save_picker(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "ピッカーを保存", f"{self.char_name_edit.text()}_picker.json", "JSON Files (*.json)"
        )
        ...
        if file_path:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(picker_data, f, ensure_ascii=False, indent=2)
                
            QtWidgets.QMessageBox.information(self, "成功", f"ピッカーを保存しました: {file_path}")
    ...
    def load_picker(self):
        ...
        if file_path:
            try:
                ...
                QtWidgets.QMessageBox.critical(self, "エラー", f"ファイルの読み込みに失敗しました: {str(e)}")
...
class PickerButton(QtWidgets.QPushButton):
    def __init__(self, controller, color, shape, size, parent=None):
        ...
        self.setText(controller.split(':')[-1].replace('_ctrl', '').replace('_control', ''))
        ...
        if shape == "円形":
            style += f"border-radius: {size//2}px;"
        elif shape == "楕円形":
...
class RigPickerUI(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle(f"{self.character_name} - リグピッカー")
        ...
        for controller, x, y, color, shape, size in self.buttons_data:
            ...
            button.setText(controller.split(':')[-1].replace('_ctrl', '').replace('_control', ''))
    ...
    def select_controller(self, controller):
        ...
        try:
            if cmds.objExists(controller):
                ...
                print(f"選択されました: {controller}")
            else:
                print(f"コントローラーが見つかりません: {controller}")
        except Exception as e:
            print(f"選択エラー: {str(e)}")
