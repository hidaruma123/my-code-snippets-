...
class PickerAreaLabel(QtWidgets.QLabel):
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton and not self.selection_origin.isNull():
            ...
            self.parent_tool.select_buttons_in_rect(self.rubber_band.geometry())
            self.selection_origin = QtCore.QPoint()
...
class PickerButton(QtWidgets.QPushButton):
    ...
    # ★★★ 新しいシグナル: ドラッグ開始をメインウィンドウに通知 ★★★
    drag_started = QtCore.Signal(object, QtCore.QPoint)

    def __init__(self, controller, color, shape, size, parent=None):
        super(PickerButton, self).__init__(parent)
        self.controller, self.color_hex, self.shape, self.size = controller, color, shape, size
        self.is_draggable, self.is_selected = True, False
    ...
    def mousePressEvent(self, event):
        if self.is_draggable:
            if event.button() == QtCore.Qt.LeftButton:
                ...
                # ★★★ ドラッグ開始シグナルを送信 ★★★
                self.drag_started.emit(self, event.globalPos())
        else:
            if event.button() == QtCore.Qt.LeftButton:
                try:
                    mods = QtWidgets.QApplication.keyboardModifiers()
                    is_shift = mods == QtCore.Qt.ShiftModifier
                    if cmds.objExists(self.controller):
                        cmds.select(self.controller, add=is_shift)
                except Exception as e:
                    print("Selection error: {}".format(str(e)))
        # ★★★ ここではsuperを呼ばない。ドラッグはメインウィンドウが制御する
        # super(PickerButton, self).mousePressEvent(event)

    # ★★★ ボタン自身のドラッグ処理は不要になったので削除 ★★★
    # def mouseMoveEvent(self, event): ...
    # def mouseReleaseEvent(self, event): ...
...
class RigPickerTool(QtWidgets.QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.setMouseTracking(True) # ★★★ ウィンドウ全体でマウス移動を検知 ★★★
    ...
    def add_picker_button(self):
        ...
        button.drag_started.connect(self.handle_drag_started) # ★★★ シグナルを接続 ★★★
        button.move(10, 10)
        button.show()
        self.picker_buttons.append(button)

    # ★★★ ここからがグループドラッグの中核ロジック ★★★
    def handle_drag_started(self, master_button, global_pos):
        # クリックされたボタンが選択されていなければ、グループドラッグは開始しない
        if not master_button.is_selected:
            return
        ...
        for button in self.picker_buttons:
            if button.is_selected:
                self.drag_initial_positions[button] = button.pos()

    def mouseMoveEvent(self, event):
        if self.is_dragging_group:
            delta = event.globalPos() - self.drag_start_pos
            for button, initial_pos in self.drag_initial_positions.items():
                button.move(initial_pos + delta)
        super(RigPickerTool, self).mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton and self.is_dragging_group:
            self.is_dragging_group = False
            self.drag_start_pos = None
            self.drag_initial_positions.clear()
        super(RigPickerTool, self).mouseReleaseEvent(event)
    # ★★★ グループドラッグロジックここまで ★★★
    ...
    def delete_selected_button(self):
        to_delete = [btn for btn in self.picker_buttons if btn.is_selected]
        if not to_delete:
        ...
        reply = QtWidgets.QMessageBox.question(self, 'Confirm Deletion', 
            "Delete {} button(s)?".format(len(to_delete)),
    ...
    def select_buttons_in_rect(self, selection_rect):
        ...
        if self.action_edit_mode.isChecked():
            if not is_shift: self.deselect_all_buttons()
            for button in self.picker_buttons:
                if selection_rect.intersects(button.geometry()):
                    button.is_selected = True
                    button.update_style()
        else:
            in_rect = [b.controller for b in self.picker_buttons if selection_rect.intersects(b.geometry())]
            if in_rect: cmds.select(in_rect, add=is_shift)
    ...
    def refresh_controller_list(self):
        ...
        controllers = sorted(list(set(c for p in patterns for c in cmds.ls(p, type='transform', long=True))))
    ...
    def load_picker(self):
        ...
        for btn_data in data.get('buttons', []):
            ...
            btn.drag_started.connect(self.handle_drag_started)
    ...
    def show_bg_context_menu(self, pos):
        ...
        set_a, clear_a = menu.addAction("Set Background Image"), menu.addAction("Clear Background Image")
        action = menu.exec_(self.picker_area_bg_label.mapToGlobal(pos))
        if action == set_a: self.prompt_set_background_image()
        elif action == clear_a: self.clear_background_image()
    ...
    def set_background_image(self, path):
        self.background_image_path, self.background_pixmap = path, QtGui.QPixmap(path)
    ...
    def update_background(self):
        if self.background_pixmap:
            ...
            self.picker_area_bg_label.setText(""), self.picker_area_bg_label.setStyleSheet("border: none;")
